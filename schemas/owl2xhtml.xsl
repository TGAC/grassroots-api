...
<!DOCTYPE xsl:stylesheet [
  ...
  <!ENTITY params	"http://grassroots.services/schema/parameters#">
...
>
  ...
  <!-- global variables and functions -->
	...
	<!-- Put IE into quirks mode -->
		<html>
			...
			<body>
				<xsl:apply-templates />
				<div id="footer">
        	<p>Part of the Grassroots API</p>
				</div>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template match="rdf:RDF">
		<header>
			<h1>The Grassroots Parameters schema</h1>
		</header>
		<nav>
		  <div>
        ...
        <select onChange="window.location.hash = document.getElementById('naviClass').value" id="naviClass">
	...
	<xsl:template name="url">
		...
		<xsl:choose>
			...
			<xsl:when test="$ns='&params;'">
				<a href="{concat($ns,$name)}">
					params:<xsl:value-of select="$name" />
				</a>
			</xsl:when>
	...
	<xsl:template name="prettyUrl">
		...
		<xsl:choose>
			...
			<xsl:when test='starts-with($name, "&params;")'>
				params:<xsl:value-of select='substring-after($name, "&params;")' />
			</xsl:when>
