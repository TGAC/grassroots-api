...
static int RunJob (const char *job_path, const char * const inline_query_s, const char * const db_name_s, const char * const output_path_s);
...
int ret = 10;
	...
	ret= RunJob (argv [1], query_filename_s, db_name_s, output_path_s);
	}
return ret;
}
...
static int RunJob (const char *job_path, const char * const query_filename_s, const char * const db_name_s, const char * const output_path_s)
{
	char diagnosis[DRMAA_ERROR_STRING_BUFFER];
	int drmaa_errno = 0;
	char job_id[MAX_LEN_JOBID];
	const char * job_argv[] = {"-db", NULL, "-query", NULL,  NULL};
	...
	drmaa_set_attribute(jt, DRMAA_WD, "/tgac/workarea/group-si/blast-2.2.30/x86_64/bin/" /* DRMAA_PLACEHOLDER_HD */, NULL, 0);
	...
	/* path for output */
	drmaa_set_attribute(jt, DRMAA_OUTPUT_PATH, output_path_s /* ":"DRMAA_PLACEHOLDER_HD"/DRMAA_JOB" */, NULL, 0);
