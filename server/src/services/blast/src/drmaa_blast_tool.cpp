...
const char * const DrmaaBlastTool :: DBT_ASYNC_S = "async";
...
{
	dbt_async_flag = true;
	if (!GetJSONBoolean (root_p, DBT_ASYNC_S, &dbt_async_flag))
		{
	...
	if (success_flag)
		{
			...
			if (json_object_set_new (root_p, DBT_ASYNC_S, dbt_async_flag ? json_true () : json_false ()) == 0)
				...
				{
					PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed add async flag %d to drmaa json for %s", dbt_async_flag, uuid_s);
				}
