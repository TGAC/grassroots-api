...
static bool AddServiceJobToServicesListManager (JobsManager *manager_p, uuid_t job_key, ServiceJob *job_p)
{
	...
	UUIDJobNode *node_p = GetServiceJobNode ((ServicesListManager *) manager_p, job_key);
...
{
	ServiceJob *job_p = RemoveServiceJobFromServicesListManager (manager_p, job_key, true);
