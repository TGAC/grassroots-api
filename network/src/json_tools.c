			...
			if (new_req_p)
				{
					...
					if (json_object_set (new_req_p, SERVICES_NAME_S, client_params_json_p) == 0)
						{
							#if JSON_TOOLS_DEBUG >= STM_LEVEL_FINER
							PrintJSONToLog (new_req_p, "Client sending: ", STM_LEVEL_FINER, __FILE__, __LINE__);
							#endif
							...
							if (services_json_p)
								{
									#if JSON_TOOLS_DEBUG >= STM_LEVEL_FINER
									PrintJSONToLog (services_json_p, "Client received: ", STM_LEVEL_FINER, __FILE__, __LINE__);
									#endif
								}
							else
								{
									PrintJSONToErrors (new_req_p, "Empty response after client sent: ", STM_LEVEL_SEVERE, __FILE__, __LINE__);
								}
