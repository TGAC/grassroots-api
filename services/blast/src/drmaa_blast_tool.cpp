...
DrmaaBlastTool :: ~DrmaaBlastTool ()
{
	...
	PrintLog (STM_LEVEL_FINEST, __FILE__, __LINE__, "Entering ~DrmaaBlastTool for %s with job id %s", buffer_s, dbt_drmaa_tool_p -> dt_id_s);
	#endif

	FreeDrmaaTool (dbt_drmaa_tool_p);

	#if DRMAA_BLAST_TOOL_DEBUG >= STM_LEVEL_FINEST
	PrintLog (STM_LEVEL_FINEST, __FILE__, __LINE__, "Exiting ~DrmaaBlastTool for %s", buffer_s);
	#endif
}
